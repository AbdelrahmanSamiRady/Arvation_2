...
import { Button, Card, CardContent, CardDescription, CardHeader, CardTitle, Input, Textarea, Badge } from "./ui";
...
export default function Page() {
  ...
  const projects = [
    { title: "Maindiish", blurb: "A social platform for food lovers to share recipes, connect, and get inspired.", href: "https://maindiish.com", tags: ["Food", "Social", "Community"] },
    { title: "The Tea News", blurb: "A modern news platform tailored for Gen Z and above, delivering quick, fresh, and relevant updates.", href: "https://www.theteanews.com", tags: ["News", "Content Automation", "AI"] },
    { title: "The Five Land", blurb: "Authentic K‑beauty e‑commerce with Shopify & marketplace integrations.", href: "https://thefiveland.com", tags: ["E‑commerce", "Operations", "Automation"] },
    { title: "Nepri.us", blurb: "Farming‑as‑a‑Service: virtual plots, AI & robotics powered agriculture.", href: "https://nepri.us", tags: ["AgTech", "AI", "Robotics"] },
    { title: "AI Lab (Arabic)", blurb: "Educational tutorials in Arabic about AI, automation, and data science.", href: "https://www.youtube.com/@3a2lSenai", tags: ["Education", "YouTube", "Community"] },
  ];

  useEffect(() => {
    try {
      console.assert(Array.isArray(projects) && projects.length >= 4, "Projects should include at least 4 items");
      projects.forEach((p) => { try { new URL(p.href); } catch { console.error("Invalid project URL:", p.title, p.href); } });
  ...
  return (
    <div id="top" className="min-h-screen bg-gradient-to-b from-white to-slate-50 dark:from-slate-950 dark:to-slate-900 text-slate-900 dark:text-slate-100">
      <header className="sticky top-0 z-40 backdrop-blur bg-white dark:bg-slate-900/70 border-b border-slate-200 dark:border-slate-800">
        <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <a href="#top" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="inline-flex items-center gap-2 group" aria-label="Go to top">
              <img src="https://maindiish-ca-1.s3.us-west-1.amazonaws.com/ArvationLogo.png" alt="Arvation Logo" className="h-9 w-auto object-contain transition-transform group-hover:scale-105" />
              <span className="font-semibold tracking-tight text-lg leading-none">Software Solution</span>
            </a>
          </div>
          <nav className="hidden md:flex items-center gap-6 text-sm">
            ...
            <a href="tel:+18186331541" className="inline-flex items-center gap-2 text-slate-700 dark:text-slate-300"><Phone className="h-4 w-4"/>+1 (818) 633‑1541</a>
            <Button onClick={toggleTheme} aria-pressed={isDark} className="ml-2 bg-white text-slate-900 border border-slate-300 dark:bg-slate-900 dark:text-white dark:border-slate-700">{isDark ? "Light" : "Dark"}</Button>
          </nav>
          <div className="md:hidden flex items-center gap-2">
            <Button onClick={toggleTheme} aria-pressed={isDark} className="bg-white text-slate-900 border border-slate-300 dark:bg-slate-900 dark:text-white dark:border-slate-700">{isDark ? "Light" : "Dark"}</Button>
        ...
        <div className="mx-auto max-w-7xl px-6 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {services.map((s) => (<Badge key={s.label} variant="secondary" className="px-3 py-1 text-sm rounded-full">{s.label}</Badge>))}
            </div>
            <div className="mt-8 flex gap-3">
              <a href="#contact"><Button>Start a project</Button></a>
              <a href="#work"><Button className="bg-white text-slate-900 border border-slate-300 dark:bg-slate-900 dark:text-white dark:border-slate-700">See our work</Button></a>
            </div>
            ...
            <Card key={label} className="border-slate-200 dark:border-slate-800">
              ...
              <CardContent className="pt-0">
                <ul className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed list-disc pl-5">
                  {label === "AI" && (<><li>Chatbots, copilots, and retrieval‑augmented apps</li><li>Fine‑tuning, embeddings, vector search</li><li>Data pipelines and analytics</li></>)}
                  {label === "Automation" && (<><li>n8n, Zapier, custom API orchestration</li><li>Back‑office process automation</li><li>Monitoring and reliability</li></>)}
                  {label === "Frontend" && (<><li>React, Next.js, Tailwind</li><li>Design systems and accessibility</li><li>Performance optimization</li></>)}
                  {label === "Backend" and (<><li>Node.js, Python, REST/GraphQL</li><li>AWS, serverless, containers</li><li>Auth, payments, multi‑tenant SaaS</li></>)}
                </ul>
              </CardContent>
            </Card>
          ))}
      ...
      <section id="about" className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid md:grid-cols-2 gap-10 items-start">
          <div>
            <h2 className="text-3:text-4xl font-bold tracking-tight">About Arvation</h2>
            <p className="mt-4 text-slate-600 dark:text-slate-300 text-lg">
          ...
          <Card className="md:col-span-2 border-slate-200 dark:border-slate-800">
            ...
            <CardContent className="pt-0">
              <form
                ...
                onSubmit={(e) => {
                  e.preventDefault();
                  const mailto = `mailto:hello@arvation.com?subject=New project inquiry from ${encodeURIComponent(form.name || "Website Visitor")}&body=${encodeURIComponent(\`Name: \${form.name}\nEmail: \${form.email}\n\n\${form.message}\`)}`;
              ...
              >
                ...
                <div className="grid gap-2">
                  ...
                  <Textarea placeholder="What are you building? Timeline? Budget range?" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="min-h-[140px]" required />
                </div>
                <div className="flex items-center gap-3">
                  <Button type="submit">Send inquiry</Button>
                  <a href="tel:+18186331541" className="inline-flex items-center gap-2 text-slate-700 dark:text-slate-300 hover:text-slate-900"><Phone className="h-4 w-4"/> Call us</a>
                </div>
              </form>
            </CardContent>
          </Card>
